...
pa_ = 360  ## значення паузи в оновленні графіку
...
wtitle = strftime("Select 4 hour 2 min %d %m %Y",local_time).' time '.strftime("%H:%M:%S",local_time).' pause = ' .pa_. 'c. cycle N '.c_p

system(sprintf("c:\\TCPU71\\Programm\\Winscp\\winscp.com /ini=nul /script=Kotel.txt"))
...
plot today_date\
   using 1:4:xtic(substr(stringcolumn(2),0,5))  every 4 ti "КотелПодача" ls 4,\
...
'' every etvmx:etvmx using 1:($6):(LabelNamePK(substr(stringcolumn(6),1,4))) w labels tc ls 2 center offset 3,1,\
\
...
\
'' using 1:($8+10) ti "Вулиця" ls 7,\
'' every etvmn:etvmn using 1:($8+12):(LabelNameWT(substr(stringcolumn(8),1,4))) w labels tc ls 7 center offset 3,0,\
\
   32,55,60,64,70
#\
#'' using 1:($5) ti "КотелПодача" ls 4,\
#'' every etvmn:etvmn using 1:($5):(LabelNameKP(substr(stringcolumn(5),1,4))) w labels tc ls 4 center offset 4,1,\
#\
#'' using 1:($5) ti "Вулиця" ls 7,\
#'' every etvmx:etvmx using 1:($5):(LabelNameWT(substr(stringcolumn(5),1,5))) w labels tc ls 7 center offset 3,0,\
